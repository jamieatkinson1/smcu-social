...
import { escapeHtml, renderShell, renderLogin, renderUnauthorised, renderDashboard, renderCommunications, renderSearchControls, renderRecordList, renderRegister, renderAssetControls, renderAssetCards, renderCommunication, renderCommunicationCampaign, renderCampaigns, renderCampaign, renderCampaignForm, renderCommunicationForm, renderCommunicationPreview, renderPublishing, renderSettings, renderAssetDetail, renderError } from "./ui.js";
...
async function start() {
  try {
    ...
    repository = await loadOperationalRepository(await loadRepository(`${base}/data/repository.json`));
...
async function loadOperationalRepository(seed){try{const snapshot=await integrationApi.authoring(),copy=structuredClone(seed);copy.campaigns=snapshot.campaigns.map(c=>({...c,title:c.name,owner:"Jamie",created:c.createdAt.slice(0,10),modified:c.updatedAt.slice(0,10),targetPublishDate:c.targetDate,priority:"Normal",assets:[],calendar:[],shopify:{},buffer:{},instagram:{},facebook:{},analytics:{},progress:{source:"checklist"},history:[],checklist:c.tasks,communications:c.communications||[]}));copy.documents=snapshot.communications.map(c=>{const seeded=seed.documents.find(document=>document.id===c.id);return {...c,documentType:c.type,department:"Communications",section:seeded?.section||"company-notices",author:"Jamie",publishDate:c.publicationDate?.slice(0,10)||null,createdDate:c.createdAt.slice(0,10),modifiedDate:c.updatedAt,summary:c.summary||"",tags:seeded?.tags||[],assets:seeded?.assets||[],shopify:{},buffer:{},analytics:{},history:[],checklist:[],notes:c.internalNotes?[c.internalNotes]:[],workflow:{needsArtwork:false,needsShopify:c.channels.includes("Shopify"),needsScheduling:c.channels.some(x=>x!=="Shopify"),needsReview:["Draft","In progress"].includes(c.status),readyToPublish:c.status==="Ready",awaitingAssets:false}}});return copy}catch{return seed}}
...
async function showCommunication() {
  if(params.get("new")==="1"){const draft={campaignId:params.get("campaign")||null,status:"Draft",type:"General update",channels:[],useMainShopify:true,useMainInstagram:true,useMainFacebook:true};mount.innerHTML=renderShell(renderCommunicationForm(draft,repository.campaigns,route),{route,active:"communications",session,breadcrumbs:[{label:"Communications",href:route("communications/index.html")},{label:"New"}]});bindCommunicationForm(null);return}
  ...
  if(params.get("edit")==="1"){mount.innerHTML=renderShell(renderCommunicationForm(communication,repository.campaigns,route),{route,active:"communications",session,breadcrumbs:[{label:"Communications",href:route("communications/index.html")},{label:communication.id}]});bindCommunicationForm(communication);return}
  if(params.get("preview")==="1"){const preview=await integrationApi.authoringPreview(communication.id);mount.innerHTML=renderShell(renderCommunicationPreview(preview,route),{route,active:"communications",session,breadcrumbs:[{label:"Communications",href:route("communications/index.html")},{label:communication.id}]});return}
  let assets;
  ...
  bindAssetActions(assets); bindArtworkManager(communication,assets);bindCommunicationActions(communication);
}
function showCampaigns() {
  if(params.get("new")==="1"){mount.innerHTML=renderShell(renderCampaignForm(null,route),{route,active:"campaigns",session});bindCampaignForm(null);return}
...
function showCampaign() {
  ...
  if(params.get("edit")==="1"){mount.innerHTML=renderShell(renderCampaignForm(campaign,route),{route,active:"campaigns",session,breadcrumbs:[{label:"Campaigns",href:route("campaigns/index.html")},{label:campaign.id}]});bindCampaignForm(campaign);return}
...
function guardUnsaved(form){let dirty=false,saving=false;form.addEventListener("input",()=>dirty=true);form.addEventListener("change",()=>dirty=true);window.addEventListener("beforeunload",event=>{if(dirty&&!saving){event.preventDefault();event.returnValue=""}});return {saved(){saving=true;dirty=false}}}
function bindCampaignForm(campaign){const form=document.querySelector("[data-campaign-form]"),list=form.querySelector("[data-task-list]"),status=form.querySelector("[data-form-status]"),guard=guardUnsaved(form);const row=()=>`<div class="task-edit" data-task><label><input type="checkbox" data-task-complete><span class="visually-hidden">Task complete</span></label><input data-task-label maxlength="160" aria-label="Campaign task"><button type="button" data-task-move="up" aria-label="Move task earlier">↑</button><button type="button" data-task-move="down" aria-label="Move task later">↓</button><button type="button" data-task-remove aria-label="Remove task">×</button></div>`;form.querySelector("[data-add-task]").addEventListener("click",()=>{list.insertAdjacentHTML("beforeend",row());list.lastElementChild.querySelector("input[data-task-label]").focus()});list.addEventListener("click",event=>{const task=event.target.closest("[data-task]");if(!task)return;if(event.target.matches("[data-task-remove]"))task.remove();if(event.target.matches("[data-task-move=up]")&&task.previousElementSibling)list.insertBefore(task,task.previousElementSibling);if(event.target.matches("[data-task-move=down]")&&task.nextElementSibling)list.insertBefore(task.nextElementSibling,task)});form.addEventListener("submit",async event=>{event.preventDefault();status.textContent="Saving campaign…";const data=new FormData(form),payload={version:Number(form.dataset.version),name:data.get("name"),purpose:data.get("purpose"),description:data.get("description"),status:data.get("status"),startDate:data.get("startDate"),targetDate:data.get("targetDate"),channels:data.getAll("channels"),notes:data.get("notes"),tasks:[...list.querySelectorAll("[data-task]")].filter(task=>task.querySelector("[data-task-label]").value.trim()).map((task,index)=>({id:`task-${index+1}`,label:task.querySelector("[data-task-label]").value,completed:task.querySelector("[data-task-complete]").checked}))};try{const result=campaign?await integrationApi.updateCampaign(campaign.id,payload):await integrationApi.createCampaign(payload);guard.saved();location.assign(route(`campaign/index.html?id=${result.campaign.id}`))}catch(error){status.textContent=`${error.message} ${error.details?.recovery||""}`.trim()}})}
function bindCommunicationForm(record){const form=document.querySelector("[data-communication-form]"),status=form.querySelector("[data-form-status]"),guard=guardUnsaved(form);form.querySelectorAll("[data-editor-wrap]").forEach(wrapper=>{const surface=wrapper.querySelector("[data-editor]");wrapper.querySelectorAll("[data-command]").forEach(button=>button.addEventListener("click",()=>{surface.focus();let value=button.dataset.value||null;if(button.dataset.command==="createLink")value=prompt("Link URL (https:// or mailto:)")||null;if(value!==null||button.dataset.command!=="createLink")document.execCommand(button.dataset.command,false,value)}));surface.addEventListener("paste",event=>{event.preventDefault();document.execCommand("insertText",false,event.clipboardData.getData("text/plain"))})});const counted=form.querySelector("[data-counted]"),counter=form.querySelector("[data-character-count]");const count=()=>counter.textContent=`${counted.value.length} / 2200`;counted.addEventListener("input",count);count();form.addEventListener("submit",async event=>{event.preventDefault();status.textContent="Saving draft…";const data=new FormData(form),dateValue=data.get("publicationDate"),payload={version:Number(form.dataset.version),title:data.get("title"),workingTitle:data.get("workingTitle"),type:data.get("type"),campaignId:data.get("campaignId")||null,status:data.get("status"),channels:data.getAll("channels"),publicationDate:dateValue?new Date(dateValue).toISOString():null,mainContentHtml:form.querySelector('[data-editor="main"]').innerHTML,summary:data.get("summary"),callToAction:data.get("callToAction"),shopifyTitle:data.get("shopifyTitle"),shopifyContentHtml:form.querySelector('[data-editor="shopify"]').innerHTML,instagramCaption:data.get("instagramCaption"),facebookCaption:data.get("facebookCaption"),promotionalCopy:data.get("promotionalCopy"),useMainShopify:data.has("useMainShopify"),useMainInstagram:data.has("useMainInstagram"),useMainFacebook:data.has("useMainFacebook"),internalNotes:data.get("internalNotes")};try{const result=record?.id?await integrationApi.updateCommunication(record.id,payload):await integrationApi.createCommunication(payload);guard.saved();location.assign(route(`document/index.html?id=${result.communication.id}`))}catch(error){status.textContent=`${error.message} ${error.details?.recovery||""}`.trim()}})}
function bindCampaignActions(campaign){document.querySelector("[data-duplicate-campaign]")?.addEventListener("click",async()=>{const result=await integrationApi.duplicateCampaign(campaign.id);location.assign(route(`campaign/index.html?id=${result.campaign.id}&edit=1`))});document.querySelector("[data-archive-campaign]")?.addEventListener("click",async()=>{if(!confirm("Archive this campaign?"))return;await integrationApi.updateCampaign(campaign.id,{...campaign,version:campaign.version,status:"Archived"});location.reload()})}
function bindCommunicationActions(record){document.querySelector("[data-duplicate-communication]")?.addEventListener("click",async()=>{const result=await integrationApi.duplicateCommunication(record.id);location.assign(route(`document/index.html?id=${result.communication.id}&edit=1`))});document.querySelector("[data-archive-communication]")?.addEventListener("click",async()=>{if(!confirm("Archive this communication?"))return;await integrationApi.updateCommunication(record.id,{...record,version:record.version,status:"Archived"});location.reload()});document.querySelector("[data-mark-ready]")?.addEventListener("click",async event=>{const output=document.querySelector("[data-ready-result]");event.currentTarget.disabled=true;try{await integrationApi.markReady(record.id,record.version);location.reload()}catch(error){output.textContent=`${error.message} ${error.details?.recovery||""}`.trim();event.currentTarget.disabled=false}})}
