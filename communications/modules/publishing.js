export function getPublishingReadiness(document, repository, suppliedAssets) {
  if (document.readiness) {
    ...
    return document.readiness.ready ? { lane: "ready", label: "Ready", reasons: [], action: "Publish" } : { lane: "blocked", label: "Blocked", reasons: document.readiness.blockers, action: null };
  }
  ...
  if (document.status === "Published") return { lane: "published", label: "Published", reasons: [], action: null };
  if (document.status === "Scheduled") return { lane: "scheduled", label: "Scheduled", reasons: [], action: null };
  if (!reasons.length && (["Ready", "Approved"].includes(document.status) || document.workflow.readyToPublish)) return { lane: "ready", label: "Ready", reasons: [], action: "Publish" };
